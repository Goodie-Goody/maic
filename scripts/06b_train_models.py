...
RESULTS_PREFIX = "v2/results_run1/" # ISOLATED GCS FOLDER FOR RUN 1
...
def compute_gasf_pytorch(x):
    ...
    # Min-Max scale to [-1, 1] per series
    ...
    # Outer product for trigonometry identity: cos(a+b) = cos(a)cos(b) - sin(a)sin(b)
...
def train_lstm(X_train, y_train, X_test, y_test, class_weights, n_classes):
    ...
    model = LSTMClassifier(
        input_dim=input_dim, hidden_dim=64, num_layers=2, n_classes=n_classes, dropout=0.5
    ).to(DEVICE)
...
def train_cnn_gaf(X_train, y_train, X_test, y_test, class_weights, n_classes, seq_length):
    ...
    model = CNNGAFClassifier(n_features=n_features, seq_length=seq_length, n_classes=n_classes).to(DEVICE)
...
def plot_shap_summary(shap_values, X_sample, feature_names, title, save_path):
    try:
        ...
        if isinstance(shap_values, list):
            shap.summary_plot(
                shap_values[2], X_sample, feature_names=feature_names, show=False, plot_type="bar"
            )
        else:
            shap.summary_plot(
                shap_values, X_sample, feature_names=feature_names, show=False, plot_type="bar"
            )
        plt.title(title)
        plt.tight_layout()
        plt.savefig(save_path, dpi=150, bbox_inches="tight")
...
def run_fold(
    gcs_client, bucket, asset, fold, train_windows, test_window, feature_cols, is_pooled=False, pooled_assets=None
):
    ...
    for binary in [False, True]:
        ...
        xgb_params = dict(
            n_estimators=300, max_depth=6, learning_rate=0.05, tree_method="hist", device="cuda" if DEVICE.type == "cuda" else "cpu",
            n_jobs=-1, random_state=42, objective="multi:softprob" if not binary else "binary:logistic",
            num_class=n_classes if not binary else None, eval_metric="mlogloss" if not binary else "logloss"
        )
        ...
        lstm_model, predictions["lstm"], lstm_preds, lstm_tl, lstm_vl = train_lstm(X_train_sc, y_train, X_test_sc, y_test, class_weights, n_classes)
        metrics["lstm"] = classification_report(y_test[SEQ_LENGTH:], lstm_preds, target_names=label_names, output_dict=True)
        plot_loss_curves(lstm_tl, lstm_vl, "LSTM", f"{tag} {mode}", f"{output_dir}/loss_lstm_{mode}.png")
        ...
        X_train_gaf, X_test_gaf = X_train_sc[:, gaf_feat_idx], X_test_sc[:, gaf_feat_idx]

        cnn_model, predictions["cnn_gaf"], cnn_preds, cnn_tl, cnn_vl = train_cnn_gaf(X_train_gaf, y_train, X_test_gaf, y_test, class_weights, n_classes, SEQ_LENGTH)
        ...
        plot_roc_curves(y_test, {k: v for k, v in predictions.items() if v is not None and len(v) == len(y_test)}, n_classes, f"ROC Curves - {tag} {mode}", f"{output_dir}/roc_{mode}.png")
        ...
        for model_name, probs in predictions.items():
            if probs is not None and len(probs) == len(y_test):
                if probs.ndim == 2:
                    for c in range(probs.shape[1]): pred_records[f"{model_name}_prob_class{c}"] = probs[:, c].tolist()
                else:
        ...
        for model_name, model_obj in [("lr", lr), ("rf", rf), ("xgb", xgb_model)]:
            with open(f"{models_dir}/{model_name}_{mode}.pkl", "wb") as f: pickle.dump({"model": model_obj, "scaler": scaler}, f)

        torch.save(lstm_model.state_dict(), f"{models_dir}/lstm_{mode}.pt")
        torch.save(cnn_model.state_dict(), f"{models_dir}/cnn_gaf_{mode}.pt")
        gc.collect()

    prefix = f"{RESULTS_PREFIX}pooled/fold_{fold}/" if is_pooled else f"{RESULTS_PREFIX}{asset}/fold_{fold}/"
...
def main():
    set_seed(42)  # LOCK THE BASELINE SEED
    ...
    for asset in ASSETS:
        ...
        for fold in range(1, n_folds + 1):
            ...
            run_fold(gcs_client, bucket, asset, fold, train_windows, test_window, feature_cols=None, is_pooled=False)
            gc.collect()
    ...
    for fold in range(1, n_folds + 1):
        ...
        test_window = fold
        logger.info(f"  Fold {fold}/{n_folds}: train={train_windows} test={test_window}")
        run_fold(gcs_client, bucket, "pooled", fold, train_windows, test_window, feature_cols=COMMON_FEATURES, is_pooled=True, pooled_assets=ASSETS)
        gc.collect()
