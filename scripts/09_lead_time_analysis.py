...
# At 0.5 the model picks up general market jitters hours before the event.
# At 0.85 we capture the acute structural break in the order book.
PROB_THRESHOLD = 0.85

# Lookback window: scan this many hours before each HMM onset.
# Narrowed to 4 hours to focus on the immediate pre-crash execution window.
LOOKBACK_HOURS = 4
...
def reconstruct_time_index(bucket, window_idx):
    ...
    This mirrors the pooled test data assembly in 06d_train_production.py:
      1. For each asset, load label files for each month in the window.
      2. Tag each row with asset_id (position in ASSETS list).
      3. Concatenate all assets and sort by time.

    The resulting time series aligns row-for-row with predictions_binary.parquet
    because both are produced by the same sort("time") operation on the same data.

    We use label files rather than feature files because they are smaller and
    contain the same time index.
    """
...
def load_predictions(bucket, fold):
    ...
    Returns a Polars DataFrame with y_true and per-model probability columns.
    """
...
def compute_lead_time(event, time_index, predictions):
    ...
    Aligns the time index with the predictions by row position, filters to
    the pre-crisis lookback window, and finds the first bar where the XGBoost
    stress probability exceeds PROB_THRESHOLD.
    """
    name  = event["name"]
    ...
    if len(time_index) != len(predictions):
        raise RuntimeError(
            ...
            f"The label files and predictions_binary.parquet are out of sync."
        )

    # Join time onto predictions by row position
    ...
    if window_df.is_empty():
        logger.warning(
            f"  [{name}] No rows in lookback window "
            f"({window_start} -> {onset})"
        )
        return {
            "event":              name,
            "fold":               event["fold"],
    ...
    # XGBoost stress probability column
    prob_col = "xgb_prob_class1"
    max_prob = round(window_df[prob_col].max(), 4)

    # Continuity check: find the first run of MIN_CONSECUTIVE_BARS bars
    # that all exceed PROB_THRESHOLD. This filters out transient spikes
    # and confirms a sustained liquidity drain is underway.
