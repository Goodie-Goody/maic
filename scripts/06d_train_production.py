...
def run_fold(gcs_client, bucket, asset, fold, train_windows, test_window,
    ...
    for binary in [False, True]:
        ...
        # ── Random Forest ────────────────────────────────────────────────────
        # Aggressive memory cleanup before RF — production hits fold 4 five times
        # (once per seed) at 18.8M rows. malloc_trim returns fragmented heap pages
        # to the OS before RF tries to allocate for 200 trees.
        import ctypes
        gc.collect()
        try:
