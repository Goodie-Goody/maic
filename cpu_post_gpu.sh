...
#
# Usage:
#   bash cpu_post_pipeline.sh             # run all stages
#   bash cpu_post_pipeline.sh --from=4    # resume from stage 4
#   bash cpu_post_pipeline.sh --dry-run   # print plan, execute nothing
#   bash cpu_post_pipeline.sh --check     # validate env/paths/imports only
#
# Full pipeline order:
#   1. bash cpu_pipeline.sh
#   2. bash gpu_pipeline.sh
#   3. bash cpu_post_pipeline.sh          (this script)
# =============================================================================
...
for arg in "$@"; do
    case $arg in
        ...
        *)
            ...
            echo "Usage: bash cpu_post_pipeline.sh [--from=N] [--dry-run] [--check]"
            exit 1
            ;;
    esac
done
...
# -----------------------------------------------------------------------------
declare -a STAGE_NUMS=(1 2 3 4 5 6)
...
# 07c requires production predictions to exist in GCS
# 09 requires fold-level predictions saved by 06d
# 10 requires production_results.csv generated by 07c
...
info "Robustness : hmm_robustness_label_agreement.csv"
info "             hmm_robustness_xgb_comparison.csv"
